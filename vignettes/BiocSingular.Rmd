...
vignette: >
  %\VignetteIndexEntry{SVD and PCA for Bioconductor}
...
epam
e.out2 <- runSVD(dummy, k=10, BSPARAM=epam)
```
...
```{r}
set.seed(1000)
i.out2 <- runSVD(dummy, k=10, BSPARAM=ipam)
```
...
```{r}
i.out2 <- runSVD(dummy, k=10, BSPARAM=ipam, BPPARAM=MulticoreParam(2))
```
...
The SVD will then be effectively performed on `t(t(dummy)-center)/scale)`, though some extra work is involved to avoid constructing this intermediate matrix.

```{r}
i.out3 <- runSVD(dummy, k=10, scale=runif(ncol(dummy)), center=rnorm(ncol(dummy)))
```

The number of left or right singular vectors to return can be directly controlled with the `nv=` and `nu=` arguments.
This can avoid wasting time in computing these vectors when they are not strictly necessary.

# Running the PCA

The main practical purpose of the SVD is to perform PCAs.
This is achieved with the `runPCA()` wrapper function, which also accepts `BSPARAM=` argument specifying the type of algorithm to use:
...
Standardization can be enabled with `scale=TRUE`.
